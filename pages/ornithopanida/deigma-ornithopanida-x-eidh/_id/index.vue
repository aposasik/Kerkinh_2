          ...
          <v-card>
            ...
            <div class="pa-4">
              ...
              <v-layout wrap justify-space-around>
                ...
                <v-spacer></v-spacer>
                <v-btn @click="toIdEditPage(deigmaXEidos.id)" color="green" dark style="text-transform: none">Επεξεργασία<v-icon small right>edit</v-icon></v-btn>
              </v-layout>
            </div>
          </v-card>
...
export default {
  head() {
    return {
      title: 'Δειγματοληψία Χ Είδους Θηλαστικών - Προβολή | Φορέας Διαχείρισης Λίμνης Κερκίνης',
      meta: [
        {
          hid: 'description', name: 'description',
          content: 'Προβολή συγκεκριμένης Δειγματοληψίας Χ Είδους Θηλαστικών για τη διαχείρηση δεδομένων των Δειγματοληψιών Χ Ειδών Θηλαστικών για τον Φορέα Διαχείρισης Λίμνης Κερκίνης.'
        }
      ]
    }
  },
  ...
  computed: {
    getByIdUrl(){
      return this.$store.getters.getOrnithopanidaXEidhByIdUrl
    },
    id(){
  ...
  methods: {
      ...
      this.$axios.get(this.getByIdUrl + id)
        .then(response => {
          ...
          if( response.data == "" ){
            ...
            this.deigmaXEidos = response.data
          }
          this.loading = false
        }).catch((err) => {
          this.loading = false
    ...
    toIdEditPage(id) {
      this.$router.push('/ornithopanida/deigma-ornithopanida-x-eidh/' + id + '/edit')
    },
    deleteById() {
