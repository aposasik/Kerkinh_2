  ...
  <v-container fluid>
    ...
    justify-center>
      ...
      <v-flex v-else-if="noElements == false" xs12 lg10 style="margin-top: 100px;">
        ...
        <div>
          <h3 class="my-4 text-xs-center text-md-left font-weight-regular">Σύνολο Ct Ειδών : {{ this.returnedData.totalResults }} </h3>
          <v-spacer></v-spacer>
        </div>
        <div v-for="ct in returnedData.ctThhlastikaEidhList" :key="ct.id" class="my-5">
          <v-card>
            ...
            <v-layout wrap justify-space-around pa-4>
              <v-flex xs12 md6 my-3>
                <v-text-field
                    label="Είδος"
                    v-model="ctornithopanida.kodikoseidos"
                    color="green"
                    readonly
                  >
                  </v-text-field>
