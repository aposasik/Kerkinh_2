  ...
  <v-container fluid>
    ...
    justify-center>
      ...
      <v-flex v-else-if="noElements == false" xs12 lg10 style="margin-top: 100px;">
        <div>
          <h3 class="my-4 text-xs-center text-md-left font-weight-regular">Σύνολο Πρωτοκόλλων : {{ this.returnedData.totalElements }} </h3>
        </div>
        <div v-for="prwtokola in returnedData.content" :key="prwtokola.id" class="my-5">
          <v-card>
            ...
            <v-layout wrap justify-space-between pa-4>
              ...
              <v-spacer></v-spacer>
              <v-btn @click="toIdPage(prwtokola.id)" color="green" dark style="text-transform: none">Εμφάνιση</v-btn>
            </v-layout>
          </v-card>
        </div>
