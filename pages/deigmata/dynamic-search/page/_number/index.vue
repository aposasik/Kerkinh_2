  ...
  <v-container fluid>
    ...
    justify-center>
      ...
      <v-flex v-else-if="noElements == false" xs12 lg10 style="margin-top: 100px;">
        ...
        <div>
          <h3 class="my-4 text-xs-center text-md-left font-weight-regular">Σύνολο Δειγμάτων : {{ this.returnedData.totalResults }} </h3>
          <v-spacer></v-spacer>
        </div>
        <div v-for="deigma in returnedData.deigmataList" :key="deigma.id" class="my-5">
          <v-card>
            ...
            <v-layout wrap justify-space-between pa-4>
              ...
              <v-flex xs12 md5 lg3>
                ...
                <p class="font-weight-regular">Id : {{ deigma.prwtokolaId }}</p>
              </v-flex>
              <v-flex v-if="deigma.deigmaThhlastikwn != null" xs12 md5 lg3>
                <h4 class="my-3 font-weight-thin"> Δείγμα Θηλαστικών </h4>
                <p class="font-weight-regular">Id : {{ deigma.deigmaThhlastikwnId }}</p>
              </v-flex>
