  ...
  <v-container fluid>
    ...
    justify-center>
      ...
      <v-flex v-else-if="noElements == false" xs12 lg9 style="margin-top: 100px;">
        <div>
          <h3 class="my-4 text-xs-center text-md-left font-weight-regular">Σύνολο Αρχείων : {{ this.returnedData.totalElements }} </h3>
        </div>

        <div v-for="file in returnedData.content" :key="file.id" class="my-5">
          <v-card>
...
export default {
  ...
  methods: {
    ...
    restCall(page) {
      this.loading = true
      this.$axios.get(this.getFiles + page)
        .then((response) => {
